 ...
 */
import { getFirstChild, isString } from "c/sfGpsDsHelpers";
import { Parser } from "./blocks";
import HtmlRenderer from "./htmlRenderer";
import HtmlUnpackFirstPRenderer from "./htmlUnpackFirstPRenderer";
import { decodeHTMLStrict } from "./entitiesDecode";
class SfGpsDsMarkdown {
    ...
    renderLinks(markdown, attribute) {
        let html = "";
        if (isString(markdown)) {
            let ast = this.parse(markdown), walker = ast.walker(), event;
            while ((event = walker.next())) {
                if (event.node.type === "link" && event.entering) {
                    if (event.node.attrs == null) {
                        event.node.attrs = [];
                    }
                    html += `<li ${attribute ? " " + attribute : ""}>${this.renderNode(event.node, attribute)}</li>`;
                }
            }
        }
        return html;
    ...
    extractLinks(markdown) {
        let links = [];
        if (isString(markdown)) {
            let ast = this.parse(markdown), walker = ast.walker(), event, index = 0;
            while ((event = walker.next())) {
                if (event.node.type === "link" && event.entering) {
                    if (event.node.attrs == null) {
                        event.node.attrs = [];
    ...
    extractH1s(markdown) {
        let h1s = [];
        if (isString(markdown)) {
            let ast = this.parse(markdown), walker = ast.walker(), event, html = "", index = 0, h1 = null, currentNode = undefined;
            while ((event = walker.next())) {
                if (event.entering) {
