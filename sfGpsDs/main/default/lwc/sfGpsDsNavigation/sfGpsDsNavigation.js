...
export default class SfGpsDsNavigation extends NavigationMixin(SfGpsDsIpLwc) {
    ...
    updateExperienceCloudNavigation() {
        if (!safeEqualsIgnoreCase(this._mode, MODE_NAV))
            return;
        ...
        getNavigationItems({
            ...
            if (data.errorMessage) {
                this.addError("EN-EM", `Issue getting the navigation: ${data.errorMessage}`);
    ...
    menuReducer(data, key, map, adaptedMap) {
        ...
        return data.reduce((m, item, index) => {
            ...
            const subNav = this.menuReducer(item.subMenu || [], itemKey, map, adaptedMap);
