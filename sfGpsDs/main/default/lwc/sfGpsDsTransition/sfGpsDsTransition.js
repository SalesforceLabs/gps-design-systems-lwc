...
import { isObject, toNumber, normaliseString, normaliseBoolean, once } from "c/sfGpsDsHelpers";
import SfGpsDsLwc from "c/sfGpsDsLwc";
import { isValidDuration, normaliseDuration, getHookArgumentsLength, nextFrame, getTimeout } from "./transition-util";
const DEBUG = false;
...
export default class SfGpsDsTransition extends SfGpsDsLwc {
    ...
    set name(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> set name value=", value);
        this._name = value;
        this.updateTransition();
        if (DEBUG)
            console.debug(CLASS_NAME, "< set name", this._name);
    }
    /* api: type */
    _type;
    _typeOriginal;
    ...
    set type(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> set type value=", value);
        ...
        if (DEBUG)
            console.debug(CLASS_NAME, "< set type", this._type);
    }
    /* api: mode */
    _mode;
    _modeOriginal;
    ...
    set mode(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> set mode value=", value);
        ...
        if (DEBUG)
            console.debug(CLASS_NAME, "< set mode", this._mode);
    }
    /* api: v-show */
    _show;
    _hasShow;
    ...
    set show(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> set show value=", value);
        if (!value !== !this._show) {
            this._show = value;
            this.updateTransition();
        }
        if (DEBUG)
            console.debug(CLASS_NAME, "< set show", this._show);
    }
    /* api: appear */
    _appear = false;
    // @ts-ignore
    ...
    set appear(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> set appear", JSON.stringify(value));
        if (value != null) {
    ...
    updateTransition() {
        if (DEBUG)
            console.debug(CLASS_NAME, "> updateTransition", this._name, this._show, this.getDisplayStyle());
        if (!this._name || this._show === undefined || this._disabled) {
            if (DEBUG)
                console.debug(CLASS_NAME, "< updateTransition not bound");
            return;
        }
        let justBound = false;
        if (!this._bound) {
        ...
        if (DEBUG)
            console.debug(CLASS_NAME, "< updateTransition");
    }
    /* classes */
    /* ------- */
    _enterClass;
    ...
    // @ts-ignore
    @api
    handleAppearCancelled;
    // for LWC compatibility, deprecated -- typo
    // @ts-ignore
    @api
    get hanndleAppearCancelled() {
        return this.handleAppearCancelled;
    }
    set hanndleAppearCancelled(hook) {
    ...
    enter(toggleDisplay) {
        ...
        const startClass = isAppear && this.appearClass ? this.appearClass : this.enterClass;
        const activeClass = isAppear && this.appearActiveClass
            ? this.appearActiveClass
            : this.enterActiveClass;
        const toClass = isAppear && this.appearToClass ? this.appearToClass : this.enterToClass;
        ...
        const explicitEnterDuration = toNumber(isObject(this.duration) ? this.duration.enter : this.duration);
    ...
    leave(rm) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> leave");
        ...
        if (this[LEAVE_CB_KEY] !== null &&
            this[LEAVE_CB_KEY] !== undefined) {
            return;
        }
        const expectsCSS = true; // this.css !== false;
        const userWantsControl = getHookArgumentsLength(this.handleLeave);
        const explicitLeaveDuration = toNumber(isObject(this.duration) ? this.duration.leave : this.duration);
    ...
    setDisplayStyle(value) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> setDisplayStyle", this._displayStyle, "->", value);
        this.hostElement.style.display = value || "none";
        this._displayStyle = value || "none";
        if (DEBUG)
            console.debug(CLASS_NAME, "< setDisplayStyle");
    }
    getDisplayStyle() {
