...
export default class SfGpsDsAuNswLwrTabBar extends SfGpsDsElement {
    ...
    focus() {
        ...
        const tab = this.querySelector(`a[data-tab-value="${this._selectedTab.value}"]`);
        if (tab) {
            tab.focus();
        }
    }
    ...
    _synchronizeA11y() {
        ...
        tabLinks.forEach((tabLink) => {
            const tabData = this._tabs.find((tab) => tabLink.getAttribute("data-tab-value") === tab.value);
            if (tabData) {
    ...
    handleKeyDown(event) {
        ...
        handleKeyDownOnTabList(event, currentFocusedIndex, {
            ...
            selectTabIndex: (index) => {
                ...
                const targetTab = this.querySelector(`a[data-tab-value="${tab.value}"]`);
                targetTab.focus();
            }
        });
    }
