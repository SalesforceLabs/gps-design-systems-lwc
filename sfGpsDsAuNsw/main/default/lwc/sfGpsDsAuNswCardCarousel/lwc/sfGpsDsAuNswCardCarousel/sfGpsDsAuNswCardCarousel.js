...
import { computeClass, styleToString, formatTemplate } from "c/sfGpsDsHelpers";
...
const I18N = {
    ...
    carouselAriaLive: "Item {selected} selected. {visible} items of {total} visible."
};
...
export default class SfGpsDsAuNswCardCarousel extends SfGpsDsElement {
    // @ts-ignore
    @api
    title = "";
    // @ts-ignore
    ...
    updateItems() {
        ...
        if (DEBUG) {
            console.debug(CLASS_NAME, "< updateItems", "_items", JSON.stringify(this._items));
        }
    }
    itemsNb = 0;
    visibleItemsNb;
    ...
    get computedNavigationClassName() {
        return {
            ...
            [`${this.navigationClassName || DEFAULT_NAVIGATION_CLASSNAME}--pagination`]: this.navigationPagination
        };
    }
    ...
    get navDots() {
        return Array.from(this.refs.navDots.querySelectorAll(`.${this.computedNavigationItemClassName}`));
    }
    get displayItems() {
        if (DEBUG) {
            console.debug(CLASS_NAME, "> displayItems", "_items", JSON.stringify(this._items));
        }
        let rv = [];
        ...
        for (let i = 0; i < rv.length; i += 1) {
            if (this.__loop) {
                const modIndex = (i + itemsNb - visibleItemsNb) % itemsNb;
                const modMin = this.selectedItem;
                const modMax = (this.selectedItem + visibleItemsNb + itemsNb) % itemsNb;
                if (modMin < modMax
        ...
        if (DEBUG) {
            console.debug(CLASS_NAME, '< displayItems', JSON.stringify(rv));
        }
        return rv;
    }
    ...
    get computedListClassName() {
        return {
            ...
            "nsw-justify-content-center": this.justifyContent && (this.itemsNb || 0) < (this.visibleItemsNb || 0)
        };
    }
    ...
    showPrevItems() {
        ...
        if (this.__loop && this.totTranslate >= 0) {
            ...
            setTimeout(() => {
                this.animating = true;
                this.selectedItem = this.getIndex(this.selectedItem - (this.visibleItemsNb || 0));
    ...
    animateList(direction) {
        ...
        if (initTranslate === this.totTranslate ||
    ...
    handleCarouselNavigationClick(event) {
        const dot = event.target.closest(`.${this.computedNavigationItemClassName}`);
    ...
    handleDragStart(event) {
        if (event.detail?.origin?.closest("." + CONTROL_CLASSNAME) ||
            event.detail?.origin?.closest("." + (this.navigationClassName || DEFAULT_NAVIGATION_CLASSNAME)) ||
    ...
    handleDragging(event) {
        ...
        const translateValue = `translateX(${translate + (this.__loop ? this.translateContainer : 0)}px)`;
        // eslint-disable-next-line dot-notation
        // @ts-ignore
        this.refs.list.style.transform = this.refs.list.style["msTransform"] =
            translateValue;
    ...
    renderedCallback() {
        if (this._initialRender) {
            if (this._items?.length && this.liveFirstItem) {
                ...
                containerWidth = Math.max(0, trackEl.clientWidth -
                ...
                if (DEBUG)
                    console.debug(CLASS_NAME, "itemWidth = ", itemWidth);
                if (containerWidth < itemWidth) {
                    this.itemOriginalWidth = containerWidth;
                    itemWidth = this.itemOriginalWidth;
                }
                this.visibleItemsNb = Math.max(1, Math.floor((containerWidth + itemMargin) / (itemWidth + itemMargin)));
                this.itemsWidth = parseFloat((((containerWidth + itemMargin) /
                    this.visibleItemsNb) - itemMargin).toFixed(1));
                if (DEBUG)
                    console.debug(CLASS_NAME, "itemsWidth = ", this.itemsWidth);
