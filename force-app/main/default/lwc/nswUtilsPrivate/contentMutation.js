...
export class ContentMutation {
    ...
    sync() {
        console.log('> ContentMutation.sync', this.component);
        if (!this.component.isConnected) {
            throw new Error(
        ...
        if (this.isNative && !this.mo) {
            console.log('> ContentMutation privateConnect');
            this.privateConnect();
        }

        for (const attrName in this.state) {
            if (Object.prototype.hasOwnProperty.call(this.state, attrName)) {
                console.log('ContentMutation privateUpdate ' + attrName);
                this.privateUpdate(attrName);
            }
        }

        if (!this.isNative) {
            console.log('ContentMutation privateUpdateLiveIds ');
    ...
    privateUpdateLiveIds() {
        const root = this.template.host ? this.template.host.getRootNode() : null;
    ...
    privateConnect() {
        console.log('ContentMutation.privateConnect', this.template, this.template.host);
        

        this.root = this.template.host ? this.template.host.getRootNode() : null;
        
        if (!this.root) {
            return; // ESC
        }

        console.log('ContentMutation.privateConnect', this.root);
