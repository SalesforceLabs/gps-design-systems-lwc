...
export default class SfGpsDsAuVic2VerticalNav extends ExpandableStateMixin(SfGpsDsElement) {
    // @ts-ignore
    @api
    title = "";
    // @ts-ignore
    ...
    mapItem(item, index, length, active) {
        if (DEBUG)
            console.debug(CLASS_NAME, "> mapItem", JSON.stringify(item), index, length, active);
        ...
        let rv = {
            ...
            items: baseItem.items && baseItem.items.length === 0 ? null : baseItem.items
        };
        if (DEBUG)
            console.debug(CLASS_NAME, "< mapItem", JSON.stringify(rv));
        return rv;
    }
    toggleId(itemId) {
    ...
    handleToggleMenuItem(event) {
        if (DEBUG) {
            console.debug(CLASS_NAME, "handleToggleMenuItem", JSON.stringify(event.detail));
        }
        this.dispatchEvent(new CustomEvent("togglemenuitem", {
            detail: event.detail
        }));
    }
    handleItemClick(event) {
        if (DEBUG) {
            console.debug(CLASS_NAME, "handleItemClick", JSON.stringify(event.detail));
        }
