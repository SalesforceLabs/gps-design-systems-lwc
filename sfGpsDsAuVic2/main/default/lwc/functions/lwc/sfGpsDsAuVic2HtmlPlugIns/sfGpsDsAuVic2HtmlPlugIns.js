const plugInLinks = {
    ...
    process: ($node, parser) => {
        ...
        if ($anchor.getAttribute("target") === "_blank" &&
            ...
            const elt = parser.parseFromString(`<body><span class="rpl-u-visually-hidden"> (opens in a new window)</span></body>`, "text/html");
            if (elt.body.tagName !== "BODY")
                elt.body.remove();
...
const plugInTables = {
    ...
    process: ($node, parser) => {
        ...
        const elt = parser.parseFromString("<body>" +
            "<div class=\"rpl-table\">" +
...
const plugInLists = {
    ...
    process: ($node) => {
        ...
        if (listTypes[type]) {
            $list.classList.add(`rpl-type-list-${$list.tagName.toLowerCase()}--${listTypes[type]}`);
        }
        return $list;
    }
};
